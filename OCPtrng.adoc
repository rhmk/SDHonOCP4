...
## Install OCP 4.1 for SAP DataHub
...
////
### Create Registry VM for mirroring SAP

1.  Install AWS python API
+
----
yum install python2-boto python2-boto3
----

2. Get VpcID
aws ec2 describe-security-groups | grep VpcId | uniq

3. Get a subnetid
aws ec2 describe-instances | grep -i subnet

You will get three different subnets, one from each availabilty zone. just pick one
////

////
### Create Your registry

. Create a new namespace for the registry
----
$ oc new-project local-registry
----

. Deploy the registry from dockhub
+
----
$ oc new-app docker.io/library/registry:2
--> Found Docker image f32a97d (8 months old) from docker.io for "docker.io/library/registry:2"

    * An image stream tag will be created as "registry:2" that will track this image
    * This image will be deployed in deployment config "registry"
    * Port 5000/tcp will be load balanced by service "registry"
      * Other containers can access this service through the hostname "registry"
    * This image declares volumes and will default to use non-persistent, host-local storage.
      You can add persistent volumes later by running 'oc set volume dc/registry --add ...'
    * WARNING: Image "docker.io/library/registry:2" runs as the 'root' user which may not be permitted by your cluster administrator

--> Creating resources ...
    imagestream.image.openshift.io "registry" created
    deploymentconfig.apps.openshift.io "registry" created
    service "registry" created
--> Success
    Application is not exposed. You can expose services to the outside world by executing one or more of the commands below:
     'oc expose svc/registry'
    Run 'oc status' to view your app.
----

. expose the registry to make it accessible from Jumphost
+
----
$ oc expose svc/registry
route.route.openshift.io/registry exposed
----
...
----
$ oc status
In project local-registry on server https://api.cluster-d217.sandbox1789.opentlc.com:6443

http://registry-local-registry.apps.cluster-d217.sandbox1789.opentlc.com to pod port 5000-tcp (svc/registry)
  dc/registry deploys istag/registry:2
...
+
----
~$ curl http://registry-local-registry.apps.cluster-d217.sandbox1789.opentlc.com/v2/
{}
----
...
+
----
$ oc set volume dc/registry --add --name=storage -t pvc --claim-size=75Gi --overwrite --path=/var/lib/registry
warning: volume "storage" did not previously exist and was not overwritten. A new volume with this name has been created instead.deploymentconfig.apps.openshift.io/registry volume updated
----

For more information see this URL: https://github.com/docker/docker.github.io/blob/master/registry/deploying.md
////
...
+
////
. Add Registry to local docker Config
+
----
sed -i -e 's@^\(registries = \[.*\)].*$@\1, '$DOCKER_REGISTRY' \]@g' /etc/containers/registries.conf
----
+
. verify that the registry is modified correctly
+
----
[root@clientvm 1 ~]# cat /etc/containers/registries.conf
...
[registries.search]
registries = ['registry.access.redhat.com', 'docker.io', 'registry.fedoraproject.org', 'quay.io', 'registry.centos.org', 'add $DOCKER_REGISTRY here']
...
. Add registry to OCP nodes
+
CAUTION: This doesn't seem to work, maybe not necessary -- only if registry is insecure !
+

On the OCP nodes, the configuration is achieved by adding the "$DOCKER_REGISTRY" to the list of .spec.registryResources.insecureRegistries in the resource image.config.openshift.io/cluster.
+
see also: https://docs.openshift.com/container-platform/4.1/openshift_images/image-configuration.html#images-configuration-file_image-configuration
+
----
...
metadata:
  ...
  creationTimestamp: "2019-11-04T11:41:04Z"
  generation: 1
  name: cluster
  resourceVersion: "11094"
  selfLink: /apis/config.openshift.io/v1/images/cluster
  uid: fb92d2eb-fef7-11e9-a8b6-02c9ebde3bb4
spec:
  registrySources:
    insecureRegistries:
    - YOUR REGISTRY HERE
...
. The cluster Operater will pick the change and execute it on the nodes
+
Check with:
+
----
for worker in `oc get nodes  | awk '/worker/{print $1}'`; do
    oc debug node/$worker -- chroot /host cat /etc/container/registries.conf
done
----
...
.. Download the SAP Data Hub Foundation file, for example: `DHFOUNDATION06_0-80004015.ZIP (SAP DATA HUB - FOUNDATION 2.6)``.
...
$ cd SAPDataHub-2.6.102-Foundation
$ ./install.sh -h
----
...
+
----
oc create secret generic awsecr --from-file=.dockerconfigjson=${HOME}/.docker/config.json --type=kubernetes.io/dockerconfigjson
----
+
NOTE: not sure if it is required to create the secret in kube-system and sdh context. Try second time SDH context only.
Also unsure if it is require to bind this to builder, tiller or other service account. I leave it out for next timeout
+
----
$ oc secrets link tiller awsecr
----
////
+
...
+
----
$ aws ec2 describe-instances --filters=Name=instance-state-name,Values=running | grep -A 2 IamInstanceProfile
                   "IamInstanceProfile": {
                        "Id": "AIPATJAXSWMVNWAWNVZI4",
                        "Arn": "arn:aws:iam::225535243050:instance-profile/sdh-06d9-5p8xk-master-profile"
--
                    "IamInstanceProfile": {
                        "Id": "AIPATJAXSWMVBDU7DHIZJ",
                        "Arn": "arn:aws:iam::225535243050:instance-profile/sdh-06d9-5p8xk-worker-profile"
--
                    "IamInstanceProfile": {
                        "Id": "AIPATJAXSWMVNWAWNVZI4",
                        "Arn": "arn:aws:iam::225535243050:instance-profile/sdh-06d9-5p8xk-master-profile"
...
aws iam list-instance-profiles | jq -r '.InstanceProfiles[] |
          select(.InstanceProfileName == "sdh-06d9-5p8xk-worker-profile") | .Roles[] |
          select(.RoleName | test("worker-role")) | "\(.RoleName)/\(.RoleId)"'
sdh-06d9-5p8xk-worker-role/AROATJAXSWMVBWNXPFLXC
----
+
Now check if the worker has the `AmazonEC2ContainerRegistryPowerUser` role attached:
+
----
aws iam list-attached-role-policies --role-name sdh-GUID-XXXX-worker-role
{
    "AttachedPolicies": [
        {
...
.. In SAP Data Hub System Management (https://vsystem-sdh.apps.sdh-${GUID}.sandboxNNN.opentlc.com/), click the *Application Configuration & Secrets* button above the search bar.
